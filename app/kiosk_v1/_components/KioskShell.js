...
import KioskCategoryButton from "./KioskCategoryButton";
import KioskProductCard from "./KioskProductCard";
import { useKioskOrderStore } from "../_lib/useKioskOrderStore";

export default function KioskShell({ categories, products, onConfirm }) {
  const setCatalog = useKioskOrderStore((state) => state.setCatalog);
  const activeCategoryId = useKioskOrderStore(
    (state) => state.activeCategoryId
  );
  const setActiveCategoryId = useKioskOrderStore(
    (state) => state.setActiveCategoryId
  );
  const categoriesFromStore = useKioskOrderStore((state) => state.categories);
  const productsFromStore = useKioskOrderStore((state) => state.products);
  const cart = useKioskOrderStore((state) => state.cart);
  const addToCart = useKioskOrderStore((state) => state.addToCart);
  const removeFromCart = useKioskOrderStore((state) => state.removeFromCart);
  const clearCart = useKioskOrderStore((state) => state.clearCart);
  const total = useKioskOrderStore((state) => state.total);
  const lastAction = useKioskOrderStore((state) => state.lastAction);
  const setLastAction = useKioskOrderStore((state) => state.setLastAction);
